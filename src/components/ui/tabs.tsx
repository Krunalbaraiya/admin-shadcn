...
export function TabsList({ children }: { children: React.ReactNode }) {
  return (
    <div
      role="tablist"
      className="flex gap-1 rounded-md border bg-white p-1 dark:border-neutral-800 dark:bg-neutral-900"
    >
      {children}
    </div>
  );
...
export function TabsContent({
  ...
  return (
    <div role="tabpanel" hidden={hidden} className={cn("mt-3", className)}>
      {children}
    </div>
  );
}
